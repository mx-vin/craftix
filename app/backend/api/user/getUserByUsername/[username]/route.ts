...
export async function GET(
  ...
  if (!username) return NextResponse.json({ error: "Username is required" }, { status: 400, headers: corsHeaders });
  ...
  try {
    ...
    return NextResponse.json({ message: "Failed to fetch user", error: err.message || err }, { status: 500, headers: corsHeaders });
  }
}